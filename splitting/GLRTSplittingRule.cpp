...
#include "GLRTSplittingRule.h"

namespace grf {

	GLRTSplittingRule::GLRTSplittingRule(double alpha) :
		alpha(alpha) {
	}

	bool GLRTSplittingRule::find_best_split(const Data& data,
		size_t node,
	...
	void GLRTSplittingRule::find_best_split_internal(const Data& data,
		const std::vector<size_t>& possible_split_vars,
		const Eigen::ArrayXXd& responses_by_sample,
	...
	void GLRTSplittingRule::find_best_split_value(const Data& data,
		size_t var,
		size_t size_node,
	...
	inline double GLRTSplittingRule::compute_logrank(size_t num_failures,
		size_t n_left,
